...
#error "TCPSTATS_DTRACE requires KDTRACE_HOOKS -- add -DKDTRACE_HOOKS to CFLAGS"
#endif
#include <sys/sdt.h>
...
/*
 * Destroy a profile. Caller must hold sx xlock.
 *
 * Fix for lock ordering bug (Section 4.4): destroy_dev() can block
 * waiting for open fds to close, so we must NOT call it while holding
 * the sx xlock. Instead, we remove the profile from the list under
 * the lock, release the lock, then call destroy_dev() and free.
 *
 * Note: the caller is responsible for releasing the sx xlock BEFORE
 * calling this function. This function operates on a profile that has
...
tcpstats_profile_destroy_unlocked(struct tcpstats_profile *prof)
{
	...
	/* destroy_dev() waits for all open fds to close -- safe outside lock */
	if (prof->dev != NULL)
		destroy_dev(prof->dev);

	log(LOG_NOTICE, "tcp_stats_kld: profile '%s' deleted\n", prof->name);
 ...
 * Caller must hold sx xlock. After this returns, the profile is
 * detached from the list and can be destroyed outside the lock.
 */
static struct tcpstats_profile *
...
tcpstats_profile_sysctl_handler(SYSCTL_HANDLER_ARGS)
{
	...
	/*
	 * arg1 points to the profile name string embedded in the
	 * tcpstats_profile struct. Find the profile from there.
	 */
	...
	if (prof != NULL) {
		...
		strlcpy(prof->filter_str, input, sizeof(prof->filter_str));
		sx_xunlock(&tcpstats_profile_lock);
		log(LOG_NOTICE,
	...
	sx_xunlock(&tcpstats_profile_lock);
	/* Profile doesn't exist -- this shouldn't happen for a per-profile
	 * sysctl, but return an error if it does */
	return (ENOENT);
}
 ...
 *
 * Parameters:
 *   field_mask - bitmask controlling which field groups to populate
 *   now        - cached sbintime from caller (avoids per-socket syscall)
 */
static void
...
tcpstats_read(struct cdev *dev, struct uio *uio, int ioflag)
{
	...
	now = getsbinuptime();
	if (tcpstats_min_read_interval_ms > 0 && sc->sc_last_read != 0) {
	...
	while ((inp = inp_next(&inpi)) != NULL) {
		 ...
		 * Every TSF_CHECK_INTERVAL sockets to amortize overhead.
		 */
		if ((nsockets & (TSF_CHECK_INTERVAL - 1)) == 0) {
			...
			/* Voluntary preemption to avoid CPU monopolization */
			kern_yield(PRI_USER);
		}
		...
		/* Cache vflag to avoid repeated pointer chase. */
		uint8_t vflag = inp->inp_vflag;

		/* IP version filter (cheap integer compare). */
		...
		/* State filtering (tp cached for fill_record below). */
		...
		tcpstats_fill_record(&rec, inp, tp, field_mask, now);
...
out:
	...
	/* Update timing stats */
	{
		sbintime_t elapsed __unused = getsbinuptime() - read_start;
